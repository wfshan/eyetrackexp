...
#here to choose exp condition  chocond = 0 none eyetracking，chocond = 1 with eyetracking                
    ...
    def preguide(self,event):
        ...
        f2 = open('eyedata\entermagtime%s_%s_%s.csv'%(self.cho,self.id,self.name),'a')
        L = ['condition','id','name','gender','age','major','trialnum','materialnum','entermagtime']
        for i in L:
            f2.write('%s,'%i)
        f2.close()
    ...
    def guide(self):
        ...
        self.g_txt.EndFont()
        button = wx.Button(self.g_panel,-1,u'实验练习',pos = ((self.ww-150)/2,600),size = (150,50))
        button.Bind(wx.EVT_LEFT_DOWN,self.exp_start)
            
    def exp_start(self,event):
        #iViewXAPI.iV_Disconnect()
        os.system('python prac-beh.py')
...
#decide which condition to be in        
        if self.cho == 0 :
            self.l = self.mlist#range(20)#____________________________________________________________________________________________________________________
...
#        labe2.SetCursor(wx.StockCursor(wx.CURSOR_BLANK))
    ...
    def condition2(self):
        ...
        if self.i == 0:
            self.Focus_panel = wx.Panel(self,-1,size = wx.DisplaySize())
            label = wx.StaticText(self.Focus_panel,-1,u'请将鼠标移至红色圆圈内开始实验',pos = ((self.ww-400)/2,220),size=(100,100))
            font = wx.Font(20,wx.DEFAULT,wx.NORMAL,wx.NORMAL)
            label.SetFont(font)
            imag = wx.Image('zhunbei.png',wx.BITMAP_TYPE_PNG).ConvertToBitmap()
            focu = wx.StaticBitmap(self.Focus_panel,-1,imag,pos = ((self.ww-50)/2,330))
...
#            focu.SetBackgroundColour('white')        
    ...
    def r_condition2(self):
        ...
        self.note2 = wx.StaticText(self.c2_panel,-1,'',((self.ww-850)/2+9+self.x+len(self.keyword)*self.hanzikuan/2,62+self.y),size = (376,95))
...
#        labe.SetCursor(wx.StockCursor(wx.CURSOR_BLANK))
    ...
    def entermag(self,event):
        ...
        self.entermagtri = 1
        t = time.time()#strftime('%Y-%m-%d-%H:%M:%S',time.localtime(time.time()))#Vs
...
#        f.close()
    ...
    def timer1(self):
        ...
        if self.tri1 == 1:
            try:
                ...
                pass
            #self.magtimebegin = time.time()
            #print 'timer1,self.magtimebegin',self.magtimebegin
            self.tri1 = 0
    ...
    def timer4(self):
        ...
        if t>0 and t<0.4:
            ...
            self.magtimeend = time.time()
            print 'timer4,self.magtimeend',self.magtimeend
            a = self.magtimeend - self.magtimebegin
            if a < 100 and self.magtimebegin > 0:
                self.magtime.append(a)
...
#        self.rightnum.Bind(wx.EVT_TEXT_ENTER,self.circle)
    ...
    def nextone(self):
        self.i += 1
        if self.i < 20:#_____________________________________________________________________________________________________________________________
            if self.cho == 0:
                self.condition1()
            else:
                self.condition2()
